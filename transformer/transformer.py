...
class TransformerOld(Model):
    def __init__(self, params):
        super(TransformerOld, self).__init__(name='transformer')
    ...
    def decode(self, targets, encoder_outputs, attention_bias, train):
        ...
        decoder_inputs = with_position_encoding(decoder_inputs, hidden_size=self.params.hidden_size)
        decoder_inputs = Dropout(self.params.layer_postprocess_dropout)(decoder_inputs)

        outputs = self.decoder_stack_network(
            [decoder_inputs,
             encoder_outputs,
             decoder_self_attention_bias,
             attention_bias])
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length):
        ...
        decoder_self_attention_bias = get_decoder_self_attention_bias_from_len(
            max_decode_length)
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            decoder_input += K.slice(timing_signal, (0, 0), (i + 1, -1))
            ...
            decoder_outputs = self.decoder_stack_network(
                [decoder_input,
                 cache.get("encoder_outputs"),
                 self_attention_bias,
                 cache.get("encoder_decoder_attention_bias")]) # cache=None
